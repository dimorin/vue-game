    ...
    mutations:{
        ...
        [OPEN_CELL](state, {row,cell}){
            ...
            function checkAround(row, cell){ // 주변 8칸 지뢰인지 검색
                ...
                if(checked.includes(row+'/'+cell)){ //체크한 칸은 검사 안하기
                    return;
                }else{
                ...
                around = around.concat([state.tableData[row][cell-1],state.tableData[row][cell],state.tableData[row][cell+1]]);
                if(state.tableData[row+1]){
                    around = around.concat([state.tableData[row+1][cell-1],state.tableData[row+1][cell],state.tableData[row+1][cell+1]]);    
                }      
                ...
                if(counted.length === 0 && row > -1){ //주변 칸에 지뢰가 하나도 없으면 주변을 열어준다. 재귀 돌려 더 검사하기
                    const near = [];
                    if(row - 1 > -1){
